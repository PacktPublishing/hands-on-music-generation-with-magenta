...
# https://bokeh.pydata.org/en/latest/docs/reference/colors.html#bokeh-colors-groups
...
# TODO add .bk-canvas-overlays { overflow: hidden !important; }
...
def get_qpm(pm):
    ...
    for tempo_change in pm.get_tempo_changes():
        if tempo_change.min() > 0 \
            ...
            if qpm:
                raise Exception("Multiple tempo changes are not supported " + str(pm.get_tempo_changes()))
            qpm = tempo_change.min()
    if not qpm:
        raise Exception("Cannot find suitable qpm in " + str(pm.get_tempo_changes()))
    return qpm
...
def get_time_signature(pm):
    ...
    if len(pm.time_signature_changes) != 1:
        raise Exception("Cannot find suitable time signature in " + str(pm.time_signature_changes))
...
def get_box_horizontal(pitch, fill_alpha, line_alpha):
    box = BoxAnnotation(bottom=pitch, top=pitch + 1, fill_color="gray", fill_alpha=fill_alpha, line_alpha=line_alpha)
    box.level = "underlay"
    return box


def get_box_vertical(bar, fill_alpha, line_alpha):
    box = BoxAnnotation(left=bar, right=bar + 1, fill_color="gray", fill_alpha=fill_alpha, line_alpha=line_alpha)
    box.level = "underlay"
    return box
...
def get_label(note):
    label = Label(x=-20, y=note + 0.2, x_units="screen", text=str(note), render_mode="css", text_font_size="8pt")
    return label
...
def draw_midi(pm):
    ...
    for bar in range(start, end + 1):
        beat_per_bar = get_time_signature(pm).denominator if TIME_SCALING == 'bar' else 1
        # change alpha each beat_per_bar (if time signature is 3/4 we'll change each 4 bars)
...
if __name__ == "__main__":
    for filename in [
        "D:\\Users\\Claire\\Documents\\MuseScore3\\Scores\\drum_kit_2019-07-17-20-24-10.mid",
        # "output/drum_kit_2019-07-17-11-53-40.mid",
        # "output/drum_kit_2019-07-17-20-24-10.mid",
        # "output/drum_kit_2019-07-16-14-59-02.mid",
    ]:
        output_file(filename.replace(".mid", ".html"))
        p = draw_file(filename)
        show(p)
